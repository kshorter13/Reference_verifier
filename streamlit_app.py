        ...
        try:
            ...
            if elements['journal']:
                try:
                    ...
                    if journal_pos != -1:
                        text_after_journal = ref_text[journal_pos + len(elements['journal']):]
            ...
            elements['extraction_errors'].append(f"General extraction error: {str(e)}")
            ...
            if results:
                ...
                for result in results:
                    ...
                    if status == 'valid':
                        ...
                        auth_details = result['authenticity_check'].get('verification_details', [])
                        ...
                        format_violations = result['format_check'].get('violations', [])
                        format_suggestions = result['format_check'].get('suggestions', [])
                        ...
                        sources_checked = result['authenticity_check'].get('sources_checked', [])
                        verification_details = result['authenticity_check'].get('verification_details', [])
